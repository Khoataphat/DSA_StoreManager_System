...
public class AmountSoldManagerTree {
    ...
    public static void sortAmountSold(List<AmountSold> list, boolean desc) {
        ...
        radixSort(list, desc); //hàm thuật toán radixSort line 564
        /*Radix Sort là một thuật toán sắp xếp không so sánh,
        thường được sử dụng để sắp xếp số nguyên.
        Nó hoạt động bằng cách phân loại các số dựa trên từng chữ số của chúng,
        từ chữ số thấp nhất đến chữ số cao nhất (hoặc ngược lại, tùy thuộc vào cách triển khai).
         */
    }

// có thuật toán sort ở đây mà chưa thấy dùng <-----
...
// có thuật toán tìm kiếm nhưng chưa thấy sử dụng <-----
...
// <-----
    ...
    //in cây nhị phân AmountSold
    public void printTree() {
        printTreeRecursive(root, "", true);
    }

    private void printTreeRecursive(AmountSoldNode node, String prefix, boolean isTail) {
        if (node != null) {
            System.out.print(prefix + (isTail ? "└── " : "├── ") + node + "\n");
